{
  title: "Installing Mac OS X Tiger on your iBook G3 Clamshell",
  description: 'All you need is a thumb drive',
  date: "2025-1-6",
  tags: [
      'retro computing',
      'macintosh'
...
There's no need to struggle with burning multiple CDs, and yes, your Blueberry and Tangerine can run Mac OS X Tiger! Follow along and you'll be swimming in Aqua in no time.
...
4. 4GB USB thumb drive (USB 2.0 is OK) [like these you can get in a color that matches your iBook](https://amzn.to/4jqqKXI) or [these sleek metal ones](https://amzn.to/49F6Bdc)
5. A [USB-A to USB-C adapter](https://amzn.to/4qFWDy0) to plug the thumb drive into your modern Mac

## Installing an SSD

First and foremost, you're going to want to replace the internal drive with a high-capacity, high-speed SSD, which means you'll need two parts:

1. [A known-compatible M.2 mSATA SSD from Transcend](https://amzn.to/4aMJrCJ) -- don't cheap out and get a brand like Dogfish, or you'll suffer from crashes on wake!
2. [A known-compatible mSATA to IDE adapter](https://amzn.to/3YogsOj)

Follow the [iFixIt guide](https://www.ifixit.com/Guide/iBook+G3+Clamshell+Hard+Drive+Replacement/116) to swap the hard drive out before beginning so you have plenty of room for activities!

Note that you'll need a couple tools for this job:
...
As you follow the guide, use one row of the crushed ice tray for each step and one column for each different screw type mentioned in the step, then go backwards to reassemble. You're guaranteed to never mix a screw up!
...
Then, plug in your USB thumb drive into your modern Mac using your USB-A to USB-C adapter.

Next, open `Terminal.app` and type the following command:
...
Note the value under `IDENTIFIER` -- that's how we'll refer to the disk going forward. In my case, it's `disk9`.
...
And your thumb drive will mount up as `Mac OS X Install DVD` on your deskop!

## Modifying the thumb drive to boot on old iBooks

Your new USB thumb drive will boot and install nicely on any Firewire equipped iBook, but if you want to boot it on older Apple hardware, you'll need to modify a file inside of the `OSInstall.mpkg`.
...
Download the [Mac OS X 10.4.11 Combo Update (PPC) .dmg file](https://support.apple.com/en-us/106535), then copy it to your thumb drive with this command (or just drag it onto the thumb drive):

```sh
cp ~/Downloads/MacOSXUpdCombo10.4.11PPC.dmg /Volumes/Mac\ OS\ X\ Install\ DVD/
```
...
Your iBook will begin to boot the installer, and you can install Mac OS X normally!

## Partitioning your drive to dual-boot OS X and OS 9

Once you're inside the installer, I recommend using `Disk Utility` to partition your drive to have two separate partitions. This way, you can hold down ⌥ (Option) after powering on your machine to choose which OS to boot.
...
## Bonus: Installing OS 9

Download the [Mac OS 9 Boot kit](https://www.macintoshrepository.org/23373-mac-os-9-2-2-boot-kit-for-booting-your-g3-g4-from-an-usb-stick), unzip the file, rename it to `.dmg`, then use `Disk Utility` to restore it to your `Mac OS 9` partition.
